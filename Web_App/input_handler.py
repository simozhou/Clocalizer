...
def profile_tailor(profiles, N):
    ...
    return np.ndarray(suited_profiles, dtype=np.float32)
...
def psiblaster(seq):
    psiblast = sp.Popen("psiblast -db ./Web_App/model/uniref50 -out_ascii_pssm temp.mat", shell=True, stdin=sp.PIPE,
                        stdout=sp.PIPE, stderr=sp.STDOUT)
    result, err = psiblast.communicate(seq)
